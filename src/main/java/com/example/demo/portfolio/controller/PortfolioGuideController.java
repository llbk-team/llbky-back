...
public class PortfolioGuideController {
    ...
    public ResponseEntity<PortfolioGuide> getGuide(
            @PathVariable Integer guideId,  // URL 경로에서 guideId 추출
            @ModelAttribute GuideSearchRequest request) {  // 쿼리 파라미터 바인딩
        
        // INFO 로그: 가이드 조회 요청 정보 기록
        log.info("가이드 조회 - guideId: {}, memberId: {}", 
            guideId, request.getMemberId());
        
        // PathVariable과 ModelAttribute 값 동기화
        // (URL 경로의 guideId를 request 객체에 설정)
        request.setGuideId(guideId);
        
        // 서비스 계층 호출: 가이드 조회
        PortfolioGuide guide = portfolioGuideService.getGuideById(request.getGuideId());
     ...
     * @return 가이드 리스트
     */
    @GetMapping("/member/{memberId}")  // GET 요청 매핑
    public ResponseEntity<List<PortfolioGuide>> getGuidesByMember(
            @PathVariable Integer memberId,  // URL 경로에서 memberId 추출
            @ModelAttribute GuideSearchRequest request) {  // 쿼리 파라미터 바인딩
        
        // INFO 로그: 회원별 가이드 목록 조회 요청
        log.info("회원별 가이드 목록 조회 - memberId: {}", memberId);
        
        // PathVariable 값을 request에 설정 (동기화)
        request.setMemberId(memberId);
        
        // 서비스 계층 호출: 회원별 가이드 목록 조회
        List<PortfolioGuide> guides = portfolioGuideService.getGuidesByMemberId(request.getMemberId());
        
        // HTTP 200 OK와 함께 가이드 목록 반환
        return ResponseEntity.ok(guides);
    }

    /**
     * ⭐ 저장된 AI 피드백 조회 (@ModelAttribute 사용)
     * GET http://localhost:8080/portfolio-guide/{guideId}/feedback?memberId=1
     * 
     * @param guideId 가이드 ID (URL 경로에서 추출)
     * @param request 검색 조건 (쿼리 파라미터 → @ModelAttribute로 매핑)
     * @return 저장된 AI 피드백 (GuideResult)
     */
